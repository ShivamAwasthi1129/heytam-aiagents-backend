...
const routingDecisionTool = defineTool({
  ...
  parameters: z.object({
    ...
    assignedSlaveAgents: z.array(z.string()).describe('List of all slave agents required/assigned for this multi-step task (e.g. ["lead-concierge", "voice-agent"]).'),
    executedActions: z.array(z.string()).describe('List of tools/actions executed during orchestration (e.g. ["sendEmail: sent message", "makeVoiceCall: called +91..."]).'),
    missingKeys: z.array(z.string()).describe('List of missing API keys or credentials needed to execute any part of the request (e.g. ["twilioAccountSid", "smtpHost"]).'),
    handoffReason: z.string().describe('Why this specific agent or group of agents was selected.'),
    extractedPayload: z.record(z.string(), z.union([z.string(), z.number(), z.boolean(), z.null(), z.array(z.string())])).describe('Clean structured data from the raw input (name, phone, email, intent, dates, requestedActions, etc.).'),
